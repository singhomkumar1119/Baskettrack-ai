...
export interface ShotDetail {
  ...
  courtX: number; // 0-100 percentage of half-court width
  courtY: number; // 0-100 percentage of half-court height
...
export interface PlayerMetrics {
  ...
  playerLoad: number; // Workload index
  activityTimeSec: {
    standing: number;
    walking: number;
    jogging: number;
  ...
  leftRightBalance: { left: number; right: number }; // percentages
...
export interface BallHandlingMetrics {
  numDribbles: number;
  dribbleFrequencyHz: number; // dribbles per sec
